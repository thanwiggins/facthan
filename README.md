...
- **Roads** - once a capital's realm is fully placed, Facthan builds a terrain-following road from
  ...
  two-tone cross-section (a plain inner surface with a distinct, biome-specific outer border - see
  Designating road materials below) and a slope-limited grade; a road that crosses a gap (water, a
  ravine) is filled completely solid so it's always safely passable.
...
- `roadAnchorOffset` (default `0`) - how many blocks in front of a structure's own bounding box its
...
- `roadWidth` (default `3`) - width, in blocks, of a road's paved surface.
...
- `roadOuterBlock` (default `minecraft:cobblestone`) - fallback block for the one-block border on
  each edge of a road's surface, for any biome with no `outer` override (see Designating road
  materials below).
- `roadBridgeBlock` (default `minecraft:oak_planks`) - fallback block used to solidly fill a road
  where it crosses a gap, for any biome with no `bridge` override.
...
Facthan bundles a `facthan_default_road_materials` datapack with an `outer` override for every
overworld biome grouped by dominant natural material (grass/dirt biomes get cobblestone, desert
gets sandstone, badlands gets red sandstone, and so on) and a `bridge` override for many of them
(derived from RoadWeaver's own per-biome wood-type hints). Replace or extend it with your own
datapack the same way you would any other Facthan datapack type.

### Gentler terrain (optional datapack)
Facthan also bundles `facthan_gentle_terrain`, a plain vanilla worldgen datapack (nothing Facthan
itself reads) that dampens overworld height variance, so large structures are less likely to end up
half-buried in a cliff or floating over a hole - `terrain_adaptation: "beard_box"` (used by every
Valarian Conquest structure) only fills small gaps under a structure's footprint, it doesn't grade
or terrace the land around it, so a big castle on genuinely rough terrain can still look wrong no
matter what Facthan does at placement time.

This works by overriding 3 of vanilla's own overworld terrain density functions
(`data/minecraft/worldgen/density_function/overworld/factor.json`, `jaggedness.json`,
`offset.json`) - the same 3 files vanilla's own "Amplified" world type overrides to make terrain
*more* dramatic. This datapack scales the same spline control points in the opposite direction
(factor x1.6, jaggedness x0.4, offset x0.55) for gentler, not flatter, terrain - sea level, biome
placement, and cave generation are untouched. It only takes effect for a **new** world with this
datapack enabled at creation time; it can't reshape terrain that's already generated. Like any
other bundled datapack, remove it (or don't enable it) if you'd rather keep vanilla terrain, or
copy and re-tune the multipliers yourself for a stronger or lighter effect.
